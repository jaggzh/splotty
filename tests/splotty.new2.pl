...
sub calculate_legend_lines_needed {
    ...
    for my $i (0..$S-1) {
        ...
        next unless is_field_enabled($field_name);
        push @enabled_fields, $i;
    }
    
    return 1 if @enabled_fields == 0;  # At least one line for "no fields enabled"
    ...
    for my $i (@enabled_fields) {
        ...
        my $display_length = length($field_name) + 15;  # glyph + value + padding + shortcut
...
sub draw_header {
    ...
    print gotorc(1,1), bg256($barbg), fg256(231), bold();
    ...
    print $label, " " x ($COLS - 1 - length($label)), reset_attrs_str(), clr_eol();
    
    # Row 2: Input line (when active) or empty
    print gotorc(2,1), bg256($barbg), fg256(231);
    if ($input_active) {
        my $prompt = " Send> ";
    ...
    if ($input_active) {
        print gotorc(3,1), bg256($barbg), fg256(231);
        print " " x ($COLS - 1), reset_attrs_str(), clr_eol();
    }
}

sub highlight_field_name {
    my ($field_name, $shortcut) = @_;
    # $DB::single=1;
    return a_fieldname() . $field_name . a_rst() unless $shortcut;
    
    # Highlight the shortcut character in the field name
    my $highlighted_name = $field_name;
    if ($highlighted_name =~ s/(\Q$shortcut\E)/x${a_hotkey()}$1${a_fieldname()}z/i) {
        return a_fieldname() . $highlighted_name . a_rst();
    } else {
        # If shortcut not found in name, just append it
        return a_fieldname() . $field_name . a_rst() . "(" . a_hotkey() . $shortcut . a_rst() . ")";
    }
}
...
sub draw_legend {
    ...
    # Handle case where no fields are enabled
    if (@enabled_fields == 0) {
        my $r = $legend_start_row;
        print gotorc($r, 1), bg256($barbg), fg256(231);
        my $msg = " No fields enabled ";
    ...
    for my $i (@enabled_fields) {
        ...
        my $highlighted_name = highlight_field_name($field_name, $shortcut);
        
        # Build display text with glyph and value
        my $display_text = sprintf("%s %s:%s  ", $g, $highlighted_name, $value_str);
